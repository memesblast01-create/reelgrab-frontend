...
export const Hero: React.FC<HeroProps> = ({ onStart, onSuccess, onError, isLoading }) => {
  ...
  const BACKEND_URL = "https://reelgrab-backend.onrender.com/extract";

  // Detect platform from URL
  useEffect(() => {
    if (!url.trim()) return;

    if (url.includes("youtube.com") || url.includes("youtu.be")) setDetectedPlatform("YouTube");
    ...
    else if (url.includes("twitter.com")) setDetectedPlatform("Twitter");
    else if (url.includes("facebook.com")) setDetectedPlatform("Facebook");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch video from backend");
      ...
      console.error(err);
      onError("Unable to process this link. Make sure it's a valid public video URL.");
    }
  };
